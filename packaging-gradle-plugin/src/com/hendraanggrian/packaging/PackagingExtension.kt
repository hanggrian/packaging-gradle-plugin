...
interface PackagingExtension {
    ...
    val windowsOptions: WindowsOptions

    /** macOS platform options (available only when running on macOS). */
    val macOptions: MacOptions

    /** Linux platform options (available only when running on Linux). */
    val linuxOptions: LinuxOptions

    /** Invoke a configurator block to modify [WindowsOptions]. */
    fun windows(action: Action<WindowsOptions>) {
        action.execute(windowsOptions)
    }

    /** Invoke a configurator block to modify [MacOptions]. */
    fun mac(action: Action<MacOptions>) {
        action.execute(macOptions)
    }

    /** Invoke a configurator block to modify [LinuxOptions]. */
    fun linux(action: Action<LinuxOptions>) {
